    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        if (trabajadorSnap.exists()) {
          ...
          const novedadesQuery = query(collection(db, 'novedades'), where('clienteId', '==', user.uid), where('numeroDocumento', '==', cedulaTrabajador), orderBy('fechaInicio', 'desc'));
          const emosQuery = query(collection(db, 'emos'), where('clienteId', '==', user.uid), where('numeroDocumento', '==', cedulaTrabajador), orderBy('fechaExamen', 'desc'));
          const perfilQuery = query(collection(db, 'perfiles_cargo'), where('clienteId', '==', user.uid), where('nombrePuesto', '==', cargoTrabajador), limit(1));
